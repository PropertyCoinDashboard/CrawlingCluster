...
def korbit_parsing_page(html_data: str) -> list[CoinSymbolSchema]:
    """
    Parse Korbit page
    """
    ...
    for market_div in market_data:
        for item_div in market_div.find_all(
            ...
            for item in item_div.find_all("div", {"class": "inline-flex items-center"}):
                time.sleep(2)
...
def bithum_parsing_page(html_data: str) -> list[CoinSymbolSchema]:
    """
    Parse Bithum page
    """
    crypto_list: list[CoinSymbolSchema] = []
    ...
    for market_div in market_data:
        for row in market_div.find_all("tr", {"class": "MarketRow_market-row__ZzE1-"}):
            time.sleep(2)
